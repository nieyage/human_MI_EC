...
EC_snRNA <- subset(snRNA,idents="cardiac endothelial cell")
integrated_data <- SplitObject(EC_snRNA, split.by = "sample")
...
integrated_data <- integrated_data %>% 
  RunUMAP(reduction = "harmony", dims = 1:30,min.dist=0.05,
          reduction.name = "umap_harmony") %>% 
  FindNeighbors(reduction = "harmony") %>% 
  FindClusters(reduction = "harmony",resolution = 1) %>% 
  identity()
...
pdf("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_EC_subcluster_umap.pdf",width=5,height=5)
...
library(ggtree);
pdf("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_EC_subcluster_harmony_tree.pdf",width=6,height=6)
ggtree(data.tree,layout = "circular") + geom_tiplab()+ geom_treescale()
dev.off()


EC_snRNA<- integrated_data
...
pdf("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_EC_subcluster_annotation_dotplot.pdf",width=12,height=8)
p1<- DotPlot(EC_snRNA, features = features,dot.scale = 3) +theme(axis.text.x=element_text(size=8,angle=90,hjust=1,vjust=0.5))
p1
p1&scale_x_discrete(labels=features_label)
p2<- DotPlot(EC_snRNA, features = EC_atlas,dot.scale = 3) +theme(axis.text.x=element_text(size=8,angle=90,hjust=1,vjust=0.5))
p2
p2&scale_x_discrete(labels=EC_atlas_label)
dev.off()

Idents(EC_snRNA)<-EC_snRNA$seurat_clusters
#####further annotation########
EC_snRNA <- RenameIdents(
  object = EC_snRNA,
  '0' = 'Capillary_Endo',
  '1' = 'Venous_Endo',
  '2' = 'notsure',
...
EC_snRNA@meta.data$subcluster<-Idents(EC_snRNA)
table(EC_snRNA$subcluster,EC_snRNA$major_labl)
...
EC_snRNA@meta.data$subcluster_CAV<-Idents(EC_snRNA)
table(EC_snRNA$subcluster_CAV,EC_snRNA$major_labl)

pdf("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_EC_subcluster_annotation_umap.pdf",width=6,height=5)
DimPlot(EC_snRNA, label = T, repel = TRUE, cols=myUmapcolors, reduction = "umap_harmony",group.by = "subcluster")
DimPlot(EC_snRNA, label = T, repel = TRUE, cols=myUmapcolors, reduction = "umap_harmony",group.by = "subcluster_CAV")
dev.off()

DefaultAssay(EC_snRNA) <- "RNA"
saveRDS(EC_snRNA,"/md01/nieyg/project/EC/human_MI/02_EC/snRNA_All_EC_harmony.rds")

# proportion for subcluster

pdf("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_All_EC_subtype_proportion.pdf",width=10,height=5)
df <- as.data.frame(EC_snRNA@meta.data)
# development_stage
df_ct <- df %>%
...
pdf("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_All_EC_not_CAV_proportion.pdf",width=10,height=5)
df <- as.data.frame(EC_snRNA@meta.data)
# development_stage
df_ct <- df %>%
...
EC_snRNA<- readRDS("/md01/nieyg/project/EC/human_MI/02_EC/snRNA_All_EC_harmony.rds")

# For ATAC subcluster 
Idents(snATAC)<- snATAC$cell_type
EC_snATAC <- subset(snATAC,idents="cardiac endothelial cell")
DefaultAssay(EC_snATAC)<- "ATAC"
...
sc <- import("scopen")
matDR <- sc$Main$scopen_dr(counts = EC_snATAC@assays$peaks@counts,
                           verbose = 1)

matDR <- t(as.matrix(matDR))

colnames(matDR) <- paste0("PC_", 1:ncol(matDR))
rownames(matDR) <- colnames(EC_snATAC@assays$peaks@counts)
EC_snATAC@reductions[['scopen']] <- CreateDimReducObject(embeddings = matDR,
                                                        assay = "peaks",
                                                       key = "PC_")
DepthCor(EC_snATAC, reduction = "scopen", n = 30)
EC_snATAC <- RunUMAP(object = EC_snATAC, 
                    reduction = 'scopen', 
                    dims = 1:30, 
                    verbose = FALSE)
...
p1 <- DimPlot(object = EC_snATAC, reduction = "umap",
              group.by = "sample") +
    xlab("UMAP1") + ylab("UMAP2")
p2 <- DimPlot(object = EC_snATAC, reduction = "umap_harmony",
...
EC_snATAC <- FindNeighbors(EC_snATAC, reduction = "harmony", dims = 1:30)
EC_snATAC <- FindClusters(EC_snATAC,graph.name ='peaks_nn', resolution = resolutions, verbose = FALSE)

library(clustree)
pdf("/md01/nieyg/project/EC/human_MI/02_EC/snATAC_EC_subcluster_resolutions_select.pdf",width=15,height=15)
clustree(EC_snATAC, prefix = "peaks_snn_res.")
plotlist <- lapply(resolutions, function(x){
    cols <- myUmapcolors
    p <- DimPlot(EC_snATAC, group.by = glue::glue("peaks_snn_res.{x}"), label = TRUE,
...
DimPlot(object = EC_snATAC,cols=myUmapcolors,reduction = "umap_harmony",  group.by = "peaks_nn_res.0.8",label = T)
DimPlot(object = EC_snATAC,cols=myUmapcolors,reduction = "umap_harmony",  group.by = "patient_group",label = T)
DimPlot(object = EC_snATAC,cols=myUmapcolors,reduction = "umap_harmony",  group.by = "patient_region",label = T)
DimPlot(object = EC_snATAC,cols=myUmapcolors,reduction = "umap_harmony",  group.by = "region",label = T)
DimPlot(object = EC_snATAC,cols=myUmapcolors,reduction = "umap_harmony",  group.by = "major_labl",label = T)
DimPlot(object = EC_snATAC,cols=myUmapcolors,reduction = "umap_harmony",  group.by = "cell_type",label = T)
dev.off();

# make tree for cluster
object<- EC_snATAC
Idents(EC_snATAC)<- EC_snATAC$peaks_nn_res.0.8
...
macs2_counts <- FeatureMatrix(
     fragments = Fragments(EC_snATAC),
     features = peak,
     cells = colnames(EC_snATAC)
     )     
...
EC_snATAC[["peaks"]] <- CreateChromatinAssay(
  counts = macs2_counts,
  #fragments = fragpath[[i]],
  fragments = Fragments(EC_snATAC),
  annotation = Annotation(EC_snATAC)
)

DefaultAssay(EC_snATAC)<-"peaks"
gene.activities <- GeneActivity(EC_snATAC,features=rownames(EC_snRNA))
# add gene activities as a new assay
EC_snATAC[["ACTIVITY"]] <- CreateAssayObject(counts = gene.activities)

DefaultAssay(EC_snATAC)<-"RNA_imputation"
# The dot plot for annotation 
features <- c("CA4","PKD1L1","BTNL9","CLIC5","RGCC","SLC9C1","LNX1","F8","CD36","MGLL",# capillary Endo
	"FAM155A","TSHZ2","SLCO2A1","ZBTB7C","KCNIP4","LYST","IGFBP5","SNTG2","POSTN",# venous Endo
	"PCSK5","ARL15","LINC00639","SMAD6","NEBL","MECOM","FUT8","PRDM16","PDZD2","SLC45A4",# Arterial Endo
...
features_label<-c(rep("capillary",10),rep("venous",9),rep("Arterial",10),rep("Endocardial",10),rep("Lymphatic",9))
features_label<- features_label[which(features%in% rownames(EC_snATAC))]
features<- features[which(features%in% rownames(EC_snATAC))]
...
p1<- DotPlot(EC_snATAC, features = features,dot.scale = 3) +theme(axis.text.x=element_text(size=8,angle=90,hjust=1,vjust=0.5))
p1
p1&scale_x_discrete(labels=features_label)
p2<- DotPlot(EC_snATAC, features = EC_atlas,dot.scale = 3) +theme(axis.text.x=element_text(size=8,angle=90,hjust=1,vjust=0.5))
p2
p2&scale_x_discrete(labels=EC_atlas_label)
dev.off()
...
EC_snATAC@meta.data$subcluster<-Idents(EC_snATAC)
table(EC_snATAC$subcluster,EC_snATAC$major_labl)
...
EC_snATAC@meta.data$subcluster_CAV<-Idents(EC_snATAC)
table(EC_snATAC$subcluster_CAV,EC_snATAC$major_labl)

pdf("/md01/nieyg/project/EC/human_MI/02_EC/snATAC_EC_subcluster_annotation_umap.pdf",width=6,height=5)
DimPlot(EC_snATAC, label = T, repel = TRUE, cols=myUmapcolors, reduction = "umap_harmony",group.by = "subcluster")
DimPlot(EC_snATAC, label = T, repel = TRUE, cols=myUmapcolors, reduction = "umap_harmony",group.by = "subcluster_CAV")
dev.off()

DefaultAssay(EC_snATAC) <- "peaks"
saveRDS(EC_snATAC,"/md01/nieyg/project/EC/human_MI/02_EC/snATAC_All_EC_harmony.rds")

EC_snATAC<- readRDS("/md01/nieyg/project/EC/human_MI/02_EC/snATAC_All_EC_harmony.rds")


# proportion for subcluster

pdf("/md01/nieyg/project/EC/human_MI/02_EC/snATAC_All_EC_subtype_proportion.pdf",width=10,height=5)
df <- as.data.frame(EC_snATAC@meta.data)
# development_stage
df_ct <- df %>%
    group_by(development_stage, subcluster_CAV) %>%
    summarise(counts = n()) %>%
...
DefaultAssay(EC_snATAC) <- "ACTIVITY"
EC_snATAC <- EC_snATAC %>% 
        NormalizeData() %>%
...
EC_snRNA <- FindVariableFeatures(EC_snRNA)


# Identify anchors
transfer.anchors <- FindTransferAnchors(reference = EC_snRNA, 
    query = EC_snATAC,
    features = VariableFeatures(object = EC_snRNA),
    reference.assay = "RNA",
    query.assay = "ACTIVITY", 
    reduction = "cca")

celltype.predictions <- TransferData(anchorset = transfer.anchors, 
	refdata = EC_snRNA$subcluster,
    weight.reduction = EC_snATAC[["scopen"]], 
    dims = 1:30)

EC_snATAC <- AddMetaData(EC_snATAC, metadata = celltype.predictions)
EC_snATAC$subcluster_annotation_correct <- EC_snATAC$predicted.id == EC_snATAC$subcluster
celltype.predictions <- TransferData(anchorset = transfer.anchors, refdata = EC_snRNA$subcluster_CAV,
    weight.reduction = EC_snATAC[["scopen"]], dims = 1:30)
EC_snATAC$predicted_CAV<- EC_snATAC$predicted.id

EC_snATAC <- AddMetaData(EC_snATAC, metadata = celltype.predictions)
EC_snATAC$subcluster_CAV_annotation_correct <- EC_snATAC$predicted.id == EC_snATAC$subcluster_CAV


EC_snATAC$predicted_CAV<- factor(EC_snATAC$predicted_CAV,levels=c("Venous_Endo","C_V","Capillary_Endo","C_A","Arterial_Endo","Endocardial_Endo","Lymphatic_Endo","notsure"))
EC_snATAC$subcluster<- factor(EC_snATAC$subcluster,levels=c("Venous_Endo","C_V","Capillary_Endo","C_A","Arterial_Endo","Endocardial_Endo","Lymphatic_Endo","notsure"))

pdf("/md01/nieyg/project/EC/human_MI/02_EC/All_EC_ATAC_predicted_Ground-truth_Umap.pdf",width=10,height=5)
p1 <- DimPlot(EC_snATAC,reduction='umap_harmony' ,cols=myUmapcolors, group.by = "predicted_CAV", label = TRUE) + NoLegend() + ggtitle("Predicted annotation")
p2 <- DimPlot(EC_snATAC,reduction='umap_harmony' ,cols=myUmapcolors,group.by = "subcluster", label = TRUE) + NoLegend() + ggtitle("Ground-truth annotation")
p1 | p2
p1 <- DimPlot(EC_snATAC,reduction='umap_harmony' ,cols=myUmapcolors, group.by = "predicted.id", label = TRUE) + NoLegend() + ggtitle("Predicted annotation")
p2 <- DimPlot(EC_snATAC,reduction='umap_harmony' ,cols=myUmapcolors,group.by = "subcluster_CAV", label = TRUE) + NoLegend() + ggtitle("Ground-truth annotation")
p1 | p2
dev.off()

# not split CAV 
predictions <- table(EC_snATAC$subcluster_CAV, EC_snATAC$predicted.id)
predictions <- predictions/rowSums(predictions)  # normalize for number of cells in each cell type
predictions <- as.data.frame(predictions)

order_Var1<- c(levels(Idents(EC_snATAC)))
order_Var2<- c(levels(Idents(EC_snATAC)))
...
correct <- length(which(EC_snATAC$subcluster_CAV == EC_snATAC$predicted.id))
incorrect <- length(which(EC_snATAC$subcluster_CAV != EC_snATAC$predicted.id))
data <- FetchData(EC_snATAC, vars = c("prediction.score.max", "subcluster_CAV_annotation_correct"))
...
saveRDS(EC_snATAC,"/md01/nieyg/project/EC/human_MI/02_EC/snATAC_All_EC_harmony_predictions.rds")


# note that we restrict the imputation to variable genes from scRNA-seq, but could impute the
# full transcriptome if we wanted to
genes.use <- VariableFeatures(EC_snRNA)
refdata <- GetAssayData(EC_snRNA, assay = "RNA", slot = "data")[genes.use, ]
...
EC_snATAC[["RNA"]] <- imputation


coembed <- merge(x = EC_snRNA, y = EC_snATAC)
...
dev.off()
saveRDS(coembed,"./02_EC/All_EC_snRNA_snATAC_merged.rds")

EC_snRNA<- readRDS("./02_EC/snRNA_All_EC_harmony.rds")
EC_snATAC<- readRDS("./02_EC/snATAC_All_EC_harmony.rds")
coembed<- readRDS("./02_EC/All_EC_snRNA_snATAC_merged.rds")

# The features between notsure and CAV in snATAC 
    EC_snATAC <- NucleosomeSignal(EC_snATAC)
    EC_snATAC <- TSSEnrichment(EC_snATAC,fast=FALSE)

pdf("/md01/nieyg/project/EC/human_MI/02_EC/snATAC_features_umap.pdf",width=24,height=20)
FeaturePlot(
  object = EC_snATAC,features = c("nCount_RNA","nFeature_RNA","TSS.enrichment","nucleosome_signal",
...
VlnPlot(object = EC_snATAC,features = c("nCount_RNA","nFeature_RNA","TSS.enrichment","nucleosome_signal"), ncol = 1, pt.size = 0)
VlnPlot(object = EC_snATAC,features = c("nCount_ATAC","nFeature_ATAC","nCount_peaks","nFeature_peaks"), ncol = 1, pt.size = 0)
VlnPlot(object = EC_snATAC,features = c("nCount_ACTIVITY","nFeature_ACTIVITY","nCounts_RNA","nFeaturess_RNA"), ncol = 1, pt.size = 0)
dev.off()
...
fc <- FoldChange(EC_snATAC, ident.1 = "CAV", ident.2 = "notsure")
...
EC_snATAC<- readRDS("./02_EC/snATAC_All_EC_harmony.rds")
EC_snATAC <- TSSEnrichment(object = EC_snATAC, fast = FALSE)
Idents(EC_snATAC) <- EC_snATAC$subcluster_CAV
EC_snATAC$TSS_group <- ifelse(EC_snATAC$subcluster_CAV == "notsure", 'notsure', 'other')
pdf("./02_EC/snATAC_notsure_TSS_distribution.pdf",width=12,height=6)
TSSPlot(EC_snATAC, group.by = 'TSS_group') + NoLegend()
TSSPlot(EC_snATAC, group.by = 'subcluster_CAV') + NoLegend()
dev.off()
