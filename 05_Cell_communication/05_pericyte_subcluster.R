...
Pericyte_snRNA <- subset(snRNA,idents="Pericyte")
integrated_data <- SplitObject(Pericyte_snRNA, split.by = "sample")
...
library(clustree)
pdf("./09_Pericyte/resolutions_select_IC_subcluster_reintergrated.pdf",width=15,height=15)
...
pdf("/md01/nieyg/project/EC/human_MI/09_Pericyte/snRNA_Pericyte_subcluster_umap.pdf",width=5,height=5)
...
library(ggtree);
pdf("/md01/nieyg/project/EC/human_MI/09_Pericyte/snRNA_Pericyte_subcluster_harmony_tree.pdf",width=6,height=6)
ggtree(data.tree,layout = "circular") + geom_tiplab()+ geom_treescale()
dev.off()

Pericyte_snRNA<- integrated_data
Idents(Pericyte_snRNA)<-Pericyte_snRNA$seurat_clusters
#####further annotation########
Pericyte_snRNA <- RenameIdents(
  object = Pericyte_snRNA,
  '0' = 'Pericyte1',
  '1' = 'Pericyte2',
  '2' = 'Pericyte1',
  '3' = 'Pericyte2',
...
Pericyte_snRNA@meta.data$subcluster<-Idents(Pericyte_snRNA)
table(Pericyte_snRNA$subcluster,Pericyte_snRNA$major_labl)

pdf("/md01/nieyg/project/EC/human_MI/09_Pericyte/snRNA_Pericyte_subcluster_annotation_umap.pdf",width=6,height=5)
DimPlot(Pericyte_snRNA, label = T, repel = TRUE, cols=myUmapcolors, reduction = "umap_harmony",group.by = "subcluster")
DimPlot(Pericyte_snRNA, label = T, repel = TRUE, cols=myUmapcolors, reduction = "umap_harmony",group.by = "major_labl")
dev.off()

# proportion for subcluster
pdf("/md01/nieyg/project/EC/human_MI/09_Pericyte/snRNA_Pericyte_subtype_proportion.pdf",width=5,height=5)
df <- as.data.frame(Pericyte_snRNA@meta.data)
# development_stage
df_ct <- df %>%
...
combined<- SCTransform(Pericyte_snRNA)
DefaultAssay(combined)<- "SCT"
combined<- ScaleData(combined)
markers <- FindAllMarkers(combined, only.pos = TRUE,logfc.threshold = 0.25)
write.csv(markers,"./09_Pericyte/Pericyte_subtype_DEG_markers-log2FC1adjP005.csv")
...
pdf("./09_Pericyte/Pericyte_subtype_markers-log2FC1adjP005_top_markers-DEG_heatmap.pdf",width=10,height=10)
...
Pericyte1<-markers[markers$cluster=="Pericyte1",7]
Pericyte2<-markers[markers$cluster=="Pericyte2",7]
Pericyte3<-markers[markers$cluster=="Pericyte3",7]
Pericyte4<-markers[markers$cluster=="Pericyte4",7]

library(pheatmap)
library(clusterProfiler)
library(org.Hs.eg.db)
Pericyte_list<- list(Pericyte1,Pericyte2,Pericyte3,Pericyte4)
subtype<- c("Pericyte1","Pericyte2","Pericyte3","Pericyte4")
all_ego<- data.frame()
pdf("./09_Pericyte/Pericyte_subtype_DEG_GO_BP.pdf")
for(i in 1:length(subtype)){
    gene<- Pericyte_list[[i]];
...
dev.off()
write.csv(all_ego,"./09_Pericyte/Allsubtype_GO.csv")
...
pdf("./09_Pericyte/All_Pericyte_subtype_GO.pdf",width=14,height=12)
...
p
dev.off()
pdf("/md01/nieyg/project/EC/human_MI/09_Pericyte/snRNA_Pericyte_subcluster_annotated_dotplot.pdf",width=10,height=3)
p1<- DotPlot(Pericyte_snRNA, features = unique(top10$gene),dot.scale = 3) +theme(axis.text.x=element_text(size=8,angle=90,hjust=1,vjust=0.5))
p1
dev.off()
DefaultAssay(Pericyte_snRNA) <- "RNA"
saveRDS(Pericyte_snRNA,"/md01/nieyg/project/EC/human_MI/09_Pericyte/snRNA_Pericyte_harmony.rds")
