...
data <- as(as.matrix(CAV_snRNA@assays$RNA@counts), 'sparseMatrix')
pd <- new('AnnotatedDataFrame', data = CAV_snRNA@meta.data)
fData <- data.frame(gene_short_name = row.names(CAV_snRNA), row.names = row.names(CAV_snRNA))
...
write.csv(deg,"./02_EC/CAV/monocle2/train_monocle_DEG.csv")

deg<- read.csv("./02_EC/CAV/monocle2/train_monocle_DEG.csv")
head(deg)

disp_table <- dispersionTable(monocle_cds)
disp.genes <- subset(disp_table, mean_expression >= 0.1 &dispersion_empirical>= 1*dispersion_fit)$gene_id

#ordering_genes<- rownames(deg)[1:1000]
ordering_genes<- rownames(deg)[which(rownames(deg)%in% disp.genes)]

monocle_cds_tmp<- setOrderingFilter(monocle_cds,ordering_genes)
pdf("./02_EC/CAV/monocle2/monocle_plot_ordering_monocle_DEG_genes.pdf")
...
monocle_cds_tmp <- reduceDimension(monocle_cds_tmp,max_components=2,method="DDRTree",residualModelFormulaStr="~donor_id") 

monocle_cds_tmp <- orderCells(monocle_cds_tmp)
pdf("./02_EC/CAV/monocle2/DEG_monocle2_CAV_subtype_pseudotime.pdf",width=6,height=6)
...
library(ggpubr)
df<- phenoData(monocle_cds)
df<- as.data.frame(df)
pdf("./02_EC/CAV/monocle2/CM_subtype_cell_density_by_pseudotime.pdf",width=10,height=5)
...
monocle_cds_tmp<- setOrderingFilter(monocle_cds,variable_feature_seurat)
pdf("./02_EC/CAV/monocle2/monocle_plot_ordering_seurat_top_vari_genes.pdf")
plot_ordering_genes(monocle_cds_tmp)
#plot_pc_variance_explained(monocle_cds, return_all = F)
dev.off()
monocle_cds_tmp <- reduceDimension(monocle_cds_tmp,max_components=2,method="DDRTree")
monocle_cds_tmp <- orderCells(monocle_cds_tmp)
pdf("./02_EC/CAV/monocle2/seurat_top_vari_CAV_subtype_pseudotime.pdf",width=6,height=6)
...
pdf("./02_EC/CAV/monocle2/monocle_plot_ordering_seurat_DEG_genes.pdf")
plot_ordering_genes(monocle_cds_tmp)
#plot_pc_variance_explained(monocle_cds, return_all = F)
dev.off()
monocle_cds_tmp <- reduceDimension(monocle_cds_tmp,max_components=2,method="DDRTree")
monocle_cds_tmp <- orderCells(monocle_cds_tmp)
pdf("./02_EC/CAV/monocle2/seurat_DEG_CAV_subtype_pseudotime.pdf",width=6,height=6)
plot_cell_trajectory(monocle_cds_tmp, color_by = "seurat_clusters",size=1)
plot_cell_trajectory(monocle_cds_tmp, color_by = "CAV",,size=1,show_backbone=T)+scale_color_manual(values=myUmapcolors)
plot_complex_cell_trajectory(monocle_cds_tmp,x=1,y=1,color_by="CAV")+scale_color_manual(values=myUmapcolors)
plot_cell_trajectory(monocle_cds_tmp, color_by = "State",,size=1)
...
disp_table <- dispersionTable(monocle_cds)
disp.genes <- subset(disp_table, mean_expression >= 0.1 &dispersion_empirical>= 1*dispersion_fit)$gene_id
monocle_cds_tmp<- setOrderingFilter(monocle_cds,disp.genes)
pdf("./02_EC/CAV/monocle2/monocle_plot_ordering_monocle_top_genes.pdf")
plot_ordering_genes(monocle_cds_tmp)
#plot_pc_variance_explained(monocle_cds, return_all = F)
dev.off()
monocle_cds_tmp <- reduceDimension(monocle_cds_tmp,max_components=2,method="DDRTree")
monocle_cds_tmp <- orderCells(monocle_cds_tmp)
pdf("./02_EC/CAV/monocle2/monocle_top_CAV_subtype_pseudotime.pdf",width=6,height=6)
plot_cell_trajectory(monocle_cds_tmp, color_by = "seurat_clusters",size=1)
plot_cell_trajectory(monocle_cds_tmp, color_by = "CAV",,size=1,show_backbone=T)+scale_color_manual(values=myUmapcolors)
plot_complex_cell_trajectory(monocle_cds_tmp,x=1,y=1,color_by="CAV")+scale_color_manual(values=myUmapcolors)
plot_cell_trajectory(monocle_cds_tmp, color_by = "State",,size=1)
...
df<- monocle_cds_tmp@phenoData@data
df<- as.data.frame(df)
pdf("./02_EC/CAV/monocle2/monocle_top_CAV_subtype_cell_density_by_pseudotime.pdf",width=10,height=5)
ggplot(df,aes(Pseudotime,colour=CAV,fill=CAV))+geom_density(bw=0.5,size=1,alpha=0.5)+theme_classic()+scale_color_manual(values=myUmapcolors)+scale_fill_manual(values=myUmapcolors)
dev.off()
...
pdf("./02_EC/CAV/monocle2/monocle_plot_ordering_CAV_marker_genes.pdf")
plot_ordering_genes(monocle_cds_tmp)
#plot_pc_variance_explained(monocle_cds, return_all = F)
dev.off()
monocle_cds_tmp <- reduceDimension(monocle_cds_tmp,max_components=2,method="DDRTree")
